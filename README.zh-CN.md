...
<p align="center">
  ...
  零 JavaScript。模块化架构：轻路由 + 6 个独立子工具。命令面由 CLI 实时生成。跨平台运行。
</p>
...
<p>4.x 版本采用模块化架构。主 <code>nong</code> 是轻量路由器，不直接引用 SkiaSharp/PDFium/PaddleOCR 等重 native 依赖。重型模块各自打成独立 <code>dotnet tool</code>，按需下载安装：</p>

<pre><code>nong (12MB，轻路由)
  ├── 内嵌轻模块（纯 .NET）：
  │     word / excel / inspect / lit / aminer
  │     metaso / genre / icons / slice / skill
  │
...
<h2>能力概览 — 命令面以 <code>nong commands --json</code> 为准</h2>

<p>当前 4.3.0 本地构建回读为 <code>167 commands available</code>。精确命令面、参数契约和版本信息始终以 CLI 实际输出为准。</p>

<h3>word — Word 文档引擎</h3>
...
<h3>chart — 统计与图表（外部工具 nong-chart）</h3>
...
<table>
  ...
  <tr><td><code>nong excel restructure</code></td><td>把多来源实验表重构为标准化总表 + 描述统计工作簿</td></tr>
...
<h3>diagram — 科学图表（外部工具 nong-diagram）</h3>
...
<h3>pdf — PDF 处理（外部工具 nong-pdf）</h3>
...
<h3>pptx — 幻灯片读写（外部工具 nong-pptx）</h3>
